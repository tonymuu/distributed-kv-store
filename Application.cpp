 ...
 *
 * DESCRIPTION: Application layer class function definitions (Revised 2020)
 **********************************/
...
Application::Application(char *infile) {
    ...
    members = (Member** ) malloc(par->EN_GPSZ * sizeof(Member *));
	...
	for( i = 0; i < par->EN_GPSZ; i++ ) {
		...
		mp1[i] = new MP1Node(members[i], par, en, log, addressOfMemberNode);
		mp2[i] = new MP2Node(members[i], par, en1, log, addressOfMemberNode);
...
void Application::deleteTest() {
	...
	for ( int i = 0; i < (int)testKVPairs.size()/2; i++ ) {
		it++;
...
void Application::readTest() {
	...
	if ( par->getcurrtime() >= (TEST_TIME + FIRST_FAIL_TIME + STABILIZE_TIME) ) {
		...
		if ( par->getcurrtime() == (TEST_TIME + FIRST_FAIL_TIME + STABILIZE_TIME) ) {
			...
			if ( count == 2 ) {
				for ( int i = 0; i < (int)nodesToFail.size(); i++ ) {
					// Fail a node
